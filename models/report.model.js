...
const getReportAnualDestinos = async() => {
    const client = await initDB();
    const result = await client.query("select array_agg(es.cod_tabla) from public.entrada_salidas es");
    client.end();
    return result
};

module.exports = { getmesreport, getReportAnualDestinos };